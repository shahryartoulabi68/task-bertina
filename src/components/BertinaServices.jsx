...
function BertinaServices() {
    return (
        <div className='bg-primary-900 space-y-2 py-8 px-4 '>
            <div className='flex gap-x-10 container p-8'>
                <div className=' md:w-1/2 text-secondary-0 mt-8 space-y-8 flex flex-col justify-center items-center md:justify-start md:items-start'>
            ...
            <div className='flex flex-col items-center font-bold justify-center'>
                <h2 className='text-secondary-0 tracking-wider text-center'>دامنه خود را در برتینا با اطمینان از مالکیت و پشتیبانی ثبت نمایید</h2>
            </div>
            <div className=' '>
                <div className='  flex items-center justify-center'>
                    <form className='flex w-full max-w-[400px] items-center justify-between bg-secondary-0 rounded-lg'>
                        <button className='btn btn--primary mr-1 px-3 flex items-center gap-x-1'>
                ...
                <div className='flex items-center gap-x-4'>
                    {
                        priceDomin.map((item) => {
                            return <span key={item.id} className={`flex items-center gap-x-1 `}>
                                <span className={`text-secondary-0 ${item.id === 4 || item.id === 3 ? "hidden md:block" : ""}`}>{persianNumber(item.price)}</span>
                                <span className={`text-orange-300 ${item.id === 4 || item.id === 3 ? "hidden md:block" : ""}`}>{item.title}</span>
                            </span>
                        })
                    }
                </div>
